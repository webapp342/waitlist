-- Complete RLS Fix for Supabase Dashboard
-- This script fixes all RLS issues identified in the Supabase dashboard
...
BEGIN
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename IN (
        ...
        EXECUTE 'DROP POLICY IF EXISTS "Allow all operations for oauth sessions" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "Allow users to view their own oauth sessions" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "Allow users to insert their own oauth sessions" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "Allow users to update their own oauth sessions" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "Allow users to delete their own oauth sessions" ON public.' || r.tablename;
    END LOOP;
END $$;
